...
namespace pEzreal.Extensions
{
    ...
    {
        public static readonly Item Youmuu = new Item(ItemId.Youmuus_Ghostblade, Player.Instance.GetAutoAttackRange());
        ...
        public static void CastE(Vector3 position)
        {
            if (E.IsReady()) E.Cast(position);
        }
            ...
            foreach (var hero in EntityManager.Heroes.Enemies.Where(hero => hero.IsValidTarget(R.Range)))
            {
                ...
                foreach (
                    ...
                    EntityManager.Heroes.Enemies.Where(
                        iHero =>
                            !iHero.IsDead && iHero.IsValidTarget(R.Range) && iHero.IsInRange(hero, R.Range)))
                {
                    if (Prediction.Position.Collision.LinearMissileCollision(iHero, startPos, endPos, R.Speed,
                        R.Width, R.CastDelay))
                        hits.Add(iHero);

                    if (hits.Count < enemies) continue;
